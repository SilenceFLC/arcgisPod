...
+ (NSDictionary*)options:(NSInteger)mode {
...
//                                      kCRToastUnderStatusBarKey                 : @(self.slideUnderSwitch.on),
    ...
    if (mode==0) {
        options[kCRToastImageKey] = [UIImage imageNamed:@"success"];
        options[kCRToastBackgroundColorKey]=[UIColor colorFromHexString:@"2ecc71"];
    }
    if (mode==1) {
        options[kCRToastImageKey] = [UIImage imageNamed:@"danger"];
        options[kCRToastBackgroundColorKey]= [UIColor colorFromHexString:@"f39c12"];
    }
    if (mode==2) {
        options[kCRToastImageKey] = [UIImage imageNamed:@"verifyerror"];
        options[kCRToastBackgroundColorKey]=[UIColor colorFromHexString:@"e74c3c"];
    }
...
{
    [self setDefaultOptions:[self options:0]];
    [self showNotificationWithMessage:text completionBlock:nil];
}

+(void)showWarningNotificationWithText:(NSString*)text
{
    [self setDefaultOptions:[self options:1]];
    [self showNotificationWithMessage:text completionBlock:nil];
}

+(void)showErrorNotificationWithText:(NSString*)text
{
    [self setDefaultOptions:[self options:2]];
    [self showNotificationWithMessage:text completionBlock:nil];
}
